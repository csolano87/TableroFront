  ...
  Mapael.prototype = {
    ...
    init: function () {
      ...
      if (
        ...
        throw new Error(
          "The map class `" + self.options.map.cssClass + "` doesn't exists",
        );
      }
      ...
      if (
        $[pluginName] &&
        $[pluginName].maps &&
        $[pluginName].maps[self.options.map.name]
      ) {
        // Mapael version >= 2.x
      ...
      self.createLegends(
        "plot",
        self.plots,
        self.paper.width / self.mapConf.width,
      );
      ...
      self.$container.on(
        "showElementsInRange." + pluginName,
        function (e, opt) {
        ...
        self.$container
          ...
          .on(fullEventName, "[data-id]", function (e) {
            ...
            if (
              ...
              elem.options.eventHandlers[eventName](
                e,
                id,
                elem.mapElem,
                elem.textElem,
    ...
    initElem: function (id, type, elem) {
      ...
      if (elem.options.text && elem.options.text.content !== undefined) {
        ...
        // Draw text
        elem.textElem = self.paper.text(
          textPosition.x,
          textPosition.y,
          elem.options.text.content,
        );
      ...
      if (elem.textElem)
        self.setHoverOptions(
          elem.textElem,
          elem.options.text.attrs,
          elem.options.text.attrsHover,
        );
    },
    ...
    initZoom: function (mapWidth, mapHeight, zoomOptions) {
        ...
        self.$map.on("touchmove." + pluginName, function (e) {
          ...
          if (e.originalEvent.touches.length === 2) {
            ...
            if (Math.abs(pinchDist - self.previousPinchDist) > 15) {
              var coord = self.mapPagePositionToXY(
                self.zoomCenterX,
                self.zoomCenterY,
              );
              zoomLevel =
                (pinchDist - self.previousPinchDist) /
                Math.abs(pinchDist - self.previousPinchDist);
              self.$container.trigger("zoom", {
                fixedCenter: true,
                level: self.zoomData.zoomLevel + zoomLevel,
                x: coord.x,
      ...
      $("body").on(
        "mouseup." +
          pluginName +
          (zoomOptions.touch ? " touchend." + pluginName : ""),
        function () {
          mousedown = false;
      ...
      self.$map
        .on(
          "mousedown." +
            pluginName +
            (zoomOptions.touch ? " touchstart." + pluginName : ""),
          function (e) {
            clearTimeout(panningMouseUpTO);
            clearTimeout(panningMouseMoveTO);
        ...
        .on(
          "mousemove." +
            pluginName +
            (zoomOptions.touch ? " touchmove." + pluginName : ""),
          function (e) {
            ...
            if (mousedown && currentLevel !== 0) {
              ...
              if (Math.abs(offsetX) > 5 || Math.abs(offsetY) > 5) {
                ...
                self.setViewBox(
                  panX,
                  panY,
                  self.currentViewBox.w,
                  self.currentViewBox.h,
                );
    ...
    mapPagePositionToXY: function (pageX, pageY) {
      ...
      var zoomFactor =
        1 / (1 + self.zoomData.zoomLevel * self.options.map.zoom.step);
      return {
    ...
    onZoomEvent: function (e, zoomOptions) {
      ...
      var previousRelativeZoomLevel =
        1 + self.zoomData.zoomLevel * self.options.map.zoom.step;
      ...
      if (zoomOptions.area !== undefined) {
        ...
        var areaMargin =
          zoomOptions.areaMargin !== undefined ? zoomOptions.areaMargin : 10;
        ...
        zoomLevel = Math.min(
          Math.floor(
            (self.mapConf.width / areaFullWidth - 1) /
              self.options.map.zoom.step,
          ),
          Math.floor(
            (self.mapConf.height / areaFullHeight - 1) /
              self.options.map.zoom.step,
          ),
        );
      } else {
        ...
        if (zoomOptions.level !== undefined) {
          if (typeof zoomOptions.level === "string") {
            ...
            if (
              zoomOptions.level.slice(0, 1) === "+" ||
              zoomOptions.level.slice(0, 1) === "-"
            ) {
              // zoomLevel is relative
              zoomLevel =
                self.zoomData.zoomLevel + parseInt(zoomOptions.level, 10);
            } else {
        ...
        if (zoomOptions.plot !== undefined) {
          ...
          if (
            zoomOptions.latitude !== undefined &&
            zoomOptions.longitude !== undefined
          ) {
            var coords = self.mapConf.getCoords(
      ...
      if (zoomLevel === 0) {
        ...
        if (
          zoomOptions.fixedCenter !== undefined &&
          zoomOptions.fixedCenter === true
        ) {
          panX =
            self.zoomData.panX +
    ...
    onUpdateEvent: function (e, opt) {
      ...
      var fnShowElement = function (elem) {
        ...
        self.setElementOpacity(
          elem,
          elem.mapElem.originalAttrs.opacity !== undefined
            ? elem.mapElem.originalAttrs.opacity
            : 1,
          animDuration,
        );
      };
      ...
      if (
        ...
        if (self.options.map.width) {
          self.createLegends(
            "plot",
            self.plots,
            self.options.map.width / self.mapConf.width,
          );
        } else {
          self.createLegends(
            "plot",
            self.plots,
            self.$map.width() / self.mapConf.width,
          );
        }
      }
    ...
    setPlotCoords: function (plot) {
      ...
      if (plot.options.x !== undefined && plot.options.y !== undefined) {
        ...
        plot.coords = self.mapConf.getCoords(
          plot.options.latitude,
          plot.options.longitude,
        );
      }
    },
    ...
    setPlotAttributes: function (plot) {
      if (plot.options.type === "square") {
        ...
        plot.options.attrs.transform =
          plot.mapElem.baseTransform + plot.options.attrs.transform;
      } else {
        // Default : circle
    ...
    isAttrsChanged: function (originalAttrs, newAttrs) {
      for (var key in newAttrs) {
        if (
          (newAttrs.hasOwnProperty(key) &&
            typeof originalAttrs[key] === "undefined") ||
    ...
    drawLegend: function (
      ...
      for (i = 0; i < legendOptions.slices.length; ++i) {
        ...
        if (legendType === "area") {
          ...
          sliceOptions[i].type === "image" ||
          sliceOptions[i].type === "svg"
        ) {
          if (sliceOptions[i].attrs.width === undefined)
        ...
        if (
          legendType === "plot" &&
          (sliceOptions[i].type === undefined ||
            sliceOptions[i].type === "circle")
        ) {
      ...
      for (i = 0; i < sliceOptions.length; ++i) {
        ...
        if (
          sliceOptions[i].display === undefined ||
          sliceOptions[i].display === true
        ) {
          if (legendType === "area") {
            if (legendOptions.mode === "horizontal") {
            ...
            if (legendOptions.mode === "horizontal") {
              x =
                ...
                scale * sliceOptions[i].attrs.r;
              y = yCenter;
            } else {
              x = legendOptions.marginLeft + scale * sliceOptions[i].attrs.r;
              y = height + scale * sliceOptions[i].attrs.r;
            }
            legendElem = legendPaper.circle(
              x,
              y,
              scale * sliceOptions[i].attrs.r,
            );
          ...
          legendLabel = legendPaper
            .text(x, y, sliceOptions[i].label)
            .attr(legendOptions.labelAttrs);

          // Update the width and height for the paper
          if (legendOptions.mode === "horizontal") {
            var currentHeight =
              legendOptions.marginBottom + legendElemBBox.height;
            width +=
              legendOptions.marginLeft +
              legendElemBBox.width +
              legendOptions.marginLeftLabel +
          ...
          if (legendOptions.hideElemsOnClick.enabled) {
            ...
            if (
              sliceOptions[i].clicked !== undefined &&
              sliceOptions[i].clicked === true
            ) {
              self.handleClickOnLegendElem(
                legendElems[i],
                i,
     ...
     */
    handleClickOnLegendElem: function (
      elem,
      id,
      legendIndex,
      legendType,
      ...
      var hiddenNewAttr =
        hidden === "0" ? { "data-hidden": "1" } : { "data-hidden": "0" };
    ...
    elemEnter: function (elem) {
      ...
      if (elem.mapElem !== undefined) {
        self.animate(
          elem.mapElem,
          elem.mapElem.attrsHover,
          elem.mapElem.attrsHover.animDuration,
        );
      }
      ...
      if (options.value !== undefined) {
        if ($.isArray(legendOptions)) {
          ...
          options = $.extend(
            ...
            {},
            options,
            self.getLegendSlice(options.value, legendOptions),
          );
        }
      }
      return options;
      ...
      for (var i = 0; i < legend.slices.length; ++i) {
        if (
          (legend.slices[i].sliceValue !== undefined &&
            value === legend.slices[i].sliceValue) ||
          (legend.slices[i].sliceValue === undefined &&
            (legend.slices[i].min === undefined ||
              value >= legend.slices[i].min) &&
            (legend.slices[i].max === undefined ||
              value <= legend.slices[i].max))
        ) {
      ...
      var computeNextStep = function () {
        ...
        if (elapsed < durationWithMargin) {
          ...
          if (zoomDir === "in" && (w > self.currentViewBox.w || w < targetW)) {
            ...
            zoomDir === "out" &&
            (w < self.currentViewBox.w || w > targetW)
          ) {
