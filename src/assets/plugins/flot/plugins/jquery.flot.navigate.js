  ...
  function initNevigation(plot, options) {
    ...
    plot.zoom = function (args) {
      ...
      for (var key in axes) {
        ...
        var offsetBelow = $.plot.saturated.saturate(
          navigationOffset.below - (axis.min - min),
        );
        var offsetAbove = $.plot.saturated.saturate(
          navigationOffset.above - (axis.max - max),
        );
    ...
    plot.pan = function (args) {
      ...
      $.each(args.axes || plot.getAxes(), function (_, axis) {
        ...
        if (d !== 0) {
          ...
          opts.offset = {
            below: saturated.saturate(
              navigationOffsetBelow + (opts.offset.below || 0),
            ),
            above: saturated.saturate(
              navigationOffsetAbove + (opts.offset.above || 0),
            ),
          };
        }
      });
    ...
    var shouldSnap = function (delta) {
      return (
        (Math.abs(delta.y) < SNAPPING_CONSTANT &&
          Math.abs(delta.x) >= SNAPPING_CONSTANT) ||
        (Math.abs(delta.x) < SNAPPING_CONSTANT &&
          Math.abs(delta.y) >= SNAPPING_CONSTANT)
      );
    };
    ...
    var lockDeltaDirection = function (delta) {
      if (
        !lockedDirection &&
        Math.max(Math.abs(delta.x), Math.abs(delta.y)) >= SNAPPING_CONSTANT
      ) {
    ...
    var restoreAxisOffset = function (axes, initialState, delta) {
      ...
      Object.keys(axes).forEach(function (axisName) {
        ...
        if (delta[axis.direction] === 0) {
          axis.options.offset.below =
            initialState[axisName].navigationOffset.below;
          axis.options.offset.above =
            initialState[axisName].navigationOffset.above;
        }
      });
    };

    var prevDelta = { x: 0, y: 0 };
    plot.smartPan = function (
      delta,
      initialState,
      panAxes,
      preventEvent,
      ...
      if (snap) {
        panHint = {
          start: {
            x:
              initialState.startPageX -
              plot.offset().left +
              plot.getPlotOffset().left,
            y:
              initialState.startPageY -
              plot.offset().top +
      ...
      Object.keys(axes).forEach(function (axisName) {
        ...
        if (d !== 0) {
          var navigationOffsetBelow = saturated.saturate(
              axis.c2p(axis.p2c(axisMin) - (p - d)) -
                axis.c2p(axis.p2c(axisMin)),
            ),
            navigationOffsetAbove = saturated.saturate(
              axis.c2p(axis.p2c(axisMax) - (p - d)) -
                axis.c2p(axis.p2c(axisMax)),
            );
      ...
      if (!preventEvent) {
        plot
          .getPlaceholder()
          .trigger("plotpan", [plot, delta, panAxes, initialState]);
      }
    };
