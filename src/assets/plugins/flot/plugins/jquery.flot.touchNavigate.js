...
(function ($) {
  ...
  function initTouchNavigation(plot, options) {
    var gestureState = {
      ...
      useManualPan =
        options.pan.interactive && options.pan.touchMode === "manual",
      smartPanLock = options.pan.touchMode === "smartLock",
      useSmartPan =
        options.pan.interactive &&
        (smartPanLock || options.pan.touchMode === "smart"),
      pan,
      pinch,
      doubleTap;
  ...
  function checkAxesForDoubleTap(plot, e, navigationState) {
    var axis = plot.getTouchedAxis(
      e.detail.firstTouch.x,
      e.detail.firstTouch.y,
    );
    if (axis[0] !== undefined) {
  ...
  function noAxisTouched(navigationState) {
    return (
      !navigationState.touchedAxis || navigationState.touchedAxis.length === 0
    );
  }
  ...
  function updateData(e, gesture, gestureState, navigationState) {
    ...
    switch (navigationState.navigationConstraint) {
      ...
      case "axisConstrained":
        ...
        gestureState.prevTapPosition[axisDir] =
          gestureState.prevPanPosition[axisDir];
